...
class CategoryMealScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    //extracting 
    var routesarg=ModalRoute.of(context).settings.arguments as Map<String , String>;
   ...
   final dishList=DUMMY_MEALS.where((element) {
     ...
     return Scaffold(
       appBar: AppBar(title: Text('${dishtitle }Recipee ',)),
      body: Container(
 child: ListView.builder(itemBuilder: (context, index){
   return  MealItem(title: dishList[index].title, 
   imageUrl: dishList[index].imageUrl,
   complexity: dishList[index].complexity,
   addordable: dishList[index].affordability,
